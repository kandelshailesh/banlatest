  ...
  con.query(usertable,[username,password],function(err,result,fields)
  {
    bcrypt.compare(password,result[0].password).then(function(resultfromhash) {
        ...
        else
        {
            ...
            res.cookie('hashpassword',hashvalue, { maxAge: 900000, httpOnly: true });
...
// var firstdate=aarthikbarsa.split('/')
 var selectdata= "SELECT * from kagroups; SELECT sum(pd.amount) as totalamount,ai.title,kag.katitle,kag.type from paymentdetails as pd inner join accountinformation as ai on pd.subaccount=ai.accountname inner join kagroups as kag on ai.title=kag.id where pd.paymentdate between ? and ? GROUP by title";
    ...
    con.query(selectdata,[req.body.ledgerinitialdate,req.body.ledgerfinaldate],function(err,result,fields)
    {
        ...
        res.render('pages/groupledger',{'initialdate':initialdate,'finaldate':finaldate,'status':status,results:result[0],results1:result[1],totalkharcha:totalkharcha,totalaamdani:totalaamdani,initialdatedetails:startdateforledger,finaldatedetails:enddateforledger});
    }
    else
    {
...
con.query(getdetailtitle,[req.body.titlename,startdateforledger,enddateforledger],function(err,result,fields)
{
    for(i=0;i<result.length;i++)
    {
        total=total+result[i].totalamount;
